    ...
    protected void startScan() {
        ...
        if (!mMainScanCycleActive) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) {
                if (isSamsung() && !mPowerManager.isInteractive()) {
                    ...
                    LogManager.d(TAG, "Using a explicit non-empty scan filter on Samsung 8.1+ with screen off");
                    filters = new ScanFilterUtils().createScanFiltersForBeaconParsers(
                            mBeaconManager.getBeaconParsers());
                }
                else {
                    if (isSamsung()) {
                        LogManager.d(TAG, "Using a wildcard scan filter on 8.1+ Samsung with screen is on.  We will switch to a non-empty filter if the screen goes off");
                        ...
                        LogManager.d(TAG, "Using a wildcard scan filter on 8.1+ on Non-Samsung");
                    }
